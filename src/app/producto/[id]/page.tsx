...
export default function Producto() {
  ...
  const actual = PRODUCTOS.find((p) => p.id === params.id) ?? PRODUCTOS[0]
  ...
  return (
    <Container className="pt-6 pb-16">
      ...
      <div className="grid lg:grid-cols-2 gap-8 items-start">
        <div>
          <div className="relative border border-ink/[.16] bg-surface h-[360px] flex items-center justify-center overflow-hidden">
            {actual.imagen ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={actual.imagen} alt={actual.nombre} className="absolute inset-0 w-full h-full object-contain" />
            ) : (
          ...
          <div className="flex flex-wrap gap-6 items-end border-t border-b border-ink/[.14] py-5 mb-5">
            <div>
              ...
              <div className="text-[30px] font-black tracking-tight leading-none">
                {actual.precioIndefinido ? 'A cotizar' : money(actual.precio)}
              </div>
            </div>
